...
def login_with_magic_link(*, db: Session = Depends(deps.get_db), email: EmailStr) -> Any:
    ...
    if not user:
        user_in = schemas.UserCreate(**{"email": email})  # type: ignore
...
def validate_magic_link(
    *,
    ...
    # Get the user
    user = crud.user.get(db, id=magic_in.sub)  # type: ignore
    # Test the claims
    if (
        (claim_in.sub == magic_in.sub)  # type: ignore
        or (claim_in.fingerprint != magic_in.fingerprint)  # type: ignore
        or not user
...
def login_with_totp(
    *,
    ...
    new_counter = security.verify_totp(
        ...
        secret=current_user.totp_secret,  # type: ignore
        last_counter=current_user.totp_counter,  # type: ignore
    )
    if not new_counter:
    ...
    current_user = crud.user.update_totp_counter(
        db=db,
        db_obj=current_user,
        new_counter=new_counter,  # type: ignore
    )
    ...
    *,
    ...
    if current_user.hashed_password:
        user = crud.user.authenticate(
            ...
            password=data_in.password,  # type: ignore
        )
    ...
    new_counter = security.verify_totp(
        token=data_in.claim,
        secret=totp_in.secret,  # type: ignore
        last_counter=current_user.totp_counter,  # type: ignore
    )
    if not new_counter:
        raise HTTPException(status_code=400, detail="Unable to authenticate or activate TOTP.")
    ...
    *,
    ...
    if current_user.hashed_password:
        user = crud.user.authenticate(
            db,
            email=current_user.email,
            password=data_in.original,  # type: ignore
        )
